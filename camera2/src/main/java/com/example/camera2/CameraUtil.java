...
public class CameraUtil {
  ...
  public static Size getPreviewSizeByRatio(
      StreamConfigurationMap map, Point displaySize, double ratio) {
    Size[] supportSize = map.getOutputSizes(SurfaceTexture.class);
    sortCamera2Size(supportSize);
    for (Size size : supportSize) {
  ...
  public static int getDefaultPreviewSizeIndex(Context context, Size[] supportSize) {
    ...
    for (int i = 0; i < supportSize.length; i++) {
      ...
      if ((supportSize[i].getHeight() == realPoint.x)
          || (supportSize[i].getWidth() <= realPoint.y
              && supportSize[i].getHeight() <= realPoint.x)) {
        return i;
      }
    }
    return 0;
