...
public class ExifTag {
  ...
  /**
   * Sets a timestamp to this tag. The method converts the timestamp with the format of "yyyy:MM:dd
   * kk:mm:ss" and calls {@link #setValue(String)}. This method will fail if the data type is not
   * {@link #TYPE_ASCII} or the component count of this tag is not 20 or undefined.
   *
   ...
   *
   * @return the value as a String, or null if the tag's value does not exist or cannot be converted
   *     to a String.
   */
   ...
   * #TYPE_UNSIGNED_BYTE}.
   *
   * @param defaultValue the byte to return if tag's value does not exist or cannot be converted to
   *     a byte.
  ...
  /**
   * Gets the value as a Rational. If there are more than 1 Rationals in this value, gets the first
   * one. This method should be used for tags of type {@link #TYPE_RATIONAL} or {@link
   * #TYPE_UNSIGNED_RATIONAL}.
   *
   * @param defaultValue the Rational to return if tag's value does not exist or cannot be converted
   *     to a Rational.
  ...
  /**
   * Gets the value as an int. If there are more than 1 ints in this value, gets the first one. This
   * method should be used for tags of type {@link #TYPE_UNSIGNED_SHORT}, {@link
   * #TYPE_UNSIGNED_LONG}.
   *
   ...
   * #TYPE_UNSIGNED_LONG}.
   *
   * @return the value as as an array of longs, or null if the tag's value does not exist or cannot
   *     be converted to an array of longs.
   */
   ...
   *
   * @param defaultValue the long to return if tag's value does not exist or cannot be converted to
   *     a long.
   ...
   * {@link #TYPE_UNSIGNED_BYTE}, {@link #TYPE_UNSIGNED_LONG}, or {@link #TYPE_UNSIGNED_SHORT}. For
   * {@link #TYPE_RATIONAL} or {@link #TYPE_UNSIGNED_RATIONAL}, call {@link #getRational(int)}
   * instead.
   *
   ...
   * @param length the maximum number of bytes to store in buffer. If length > component count, only
   *     the valid bytes will be stored.
  ...
  protected void getBytes(byte[] buf, int offset, int length) {
    ...
    System.arraycopy(
        mValue, 0, buf, offset, (length > mComponentCountActual) ? mComponentCountActual : length);
  }
