...
import com.example.camera2.exif.ExifInterface;
import java.io.File;
...
public class FileSaver {
  ...
  private KanCamera2CallBack mListener;
  ...
  public FileSaver(Context context, KanCamera2CallBack callBack, Handler handler) {
  ...
  public void saveFile(
      int width, int height, int orientation, byte[] data, String tag, int saveType) {
  ...
  public void saveVideoFile(int width, int height, int orientation, final String path, int type) {
    ...
    if (mListener != null) {
      mHandler.post(
          new Runnable() {
            @Override
            public void run() {
              mListener.onVideoSaved(uri, path);
            }
          });
    }
  }
  ...
  private Bitmap rotateAndWriteJpegData(ExifInterface exif, ImageInfo info) {
    ...
    Bitmap rotatedMap =
        Bitmap.createBitmap(origin, 0, 0, origin.getWidth(), origin.getHeight(), matrix, true);
  ...
  private Bitmap getThumbnail(ImageInfo info) {
    if (JPEG.equals(info.imgMimeType)) {
      ...
      return BitmapFactory.decodeByteArray(info.imgData, 0, info.imgData.length, options);
    } else {
      return null;
    }
  }
